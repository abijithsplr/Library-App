...
let titleReg = /^([A-Za-z\s]+)$/;
let authornameReg = /^([A-Za-z\.\s]+)$/;
let genreReg = /^([A-Za-z\s]+)$/;

let dobReg = /^([0-2][0-9]|(3)[0-1])(\s)(((0)[0-9])|((1)[0-2]))(\s)\d{4}$/;
let languageReg = /^([A-Za-z\s]+)$/;
...
function check() {
  if (validateEmail() && validatePassword() && re_enterPassword()) {
    setAction.setAttribute('action', '/login');
    (document.getElementById("field")).style.visibility = "hidden";
    erorReenter.innerHTML = " Re-enter the password";
    erorReenter.style.color = "#6c757d";
    return true;
...
function addbookcheck() {
  ...
  else if (booktitle.value.trim() == "" || authorname.value.trim() == "" || genreReg.value.trim() == "" || cover.value.trim() == "" ) {
    (document.getElementById("field")).style.visibility = "visible";
    (document.getElementById("field")).innerHTML = "*Fields cannot be empty";
    return false;
  }
